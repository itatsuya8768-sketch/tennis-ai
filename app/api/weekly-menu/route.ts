...
export async function GET() {
  try {
    ...
    const { data: cached } = await supabase
      ...
      .maybeSingle();
    if (cached && cached.diagnosis_count === diagnoses.length) {
...
- 改善メニューは3〜4項目。各項目は「課題」と「来週の具体的な練習・意識ポイント」をセットで、平易な言葉で2文以内・100文字程度にまとめる。
- 怪我リスクが「中」以上の指摘があれば、必ず1項目はケア・予防に当てる。
...
必ずJSON形式のみで返してください：
{
  "summary": "今週の総括を1〜2文で",
  "menu": [
    { "title": "項目タイトル（10文字程度）", "issue": "課題（今週の診断で見えたこと）", "action": "来週の具体的な練習・意識ポイント" }
  ]
}`;
