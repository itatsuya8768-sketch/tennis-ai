  ...
  try {
    ...
    const takebackDesc =
      takeback?.verdict === "over"
        ? `\n【骨格計測による確定情報：テイクバック＝引きすぎ】スイング全体の骨格座標を解析した結果、ラケットを最も引いたフレームで、推定ラケットヘッド（利き腕の肘→手首の延長で算出）が${takeback.shoulderLabel}のラインより明確に後方（背中側）にありました（胴体長比 約${Math.round((takeback.beyondRatio ?? 0) * 100)}%）。これは骨格座標による客観計測であり、映像の見た目より信頼できる。診断では「テイクバック時にラケットが身体より後ろまで引かれている＝引きすぎ（オーバーテイクバック）」と必ず指摘し、コンパクトに直す改善策を述べること。「コンパクトに引けている」とは絶対に書かないこと。`
