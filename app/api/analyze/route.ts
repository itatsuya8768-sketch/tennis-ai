...
2. impactCheck：動画からインパクトの瞬間を特定し打点の位置（高さ・前後）を正確に評価。【contactPositionはテイクバックの判定から演繹してはいけない（最重要・誤診防止）】「テイクバックが引きすぎ（オーバーテイクバック）だから打点も後ろにずれているはず」のように、テイクバックの判定結果から打点位置を推論することを厳しく禁止する。テイクバックの引きすぎと打点位置（前/後ろ）は別々の観察項目であり、それぞれ独立に、実際にラケットとボールが接触している局面の画像を見て個別に判定すること。テイクバックが引きすぎでも、実際の画像で打点が身体の前で捉えられているのが確認できれば、必ず「前で捉えている」と正直に書く（テイクバックの欠点と打点の良し悪しは別に評価し、一方が悪いからといって他方も悪いと決めつけない）。【打点位置と振り遅れ判定は矛盾させない（重要）】振り遅れ・振り急ぎは「打点が身体に対してどこにあったか」の結果であり、observationsのcontactPositionと整合させること。打点が「前で捉えている」場合は振り遅れではない（むしろ良いタイミング）。「振り遅れ」と判定するのは、contactPositionが「やや後ろ」または「後ろ」で、かつ準備の遅れ・体の開きの早さなど振り遅れを示す根拠が動画で確認できた場合に限る。打点が前なのに「振り遅れ」と書く・打点が後ろなのに「良いタイミング」と書くような矛盾を禁止する。ラケット面の角度については、【ラケット面の角度に触れるときは、初心者にも分かるよう必ず平易な言葉を添える】「やや上向き（オープン＝面が少し空を向く／ボールが浮きやすい）」「垂直（スクエア＝面が真っすぐ前／まっすぐ飛ぶ）」「やや下向き（かぶせ＝面がボールに被さる／下・ネット方向に行きやすい）」のように、向きと結果をセットで説明する。専門用語だけで終わらせない。【面の角度は実際に見えた時だけ・推測で「かぶせ」と書かない】インパクト前後で面の傾きが確認できた場合のみ向きを断定し、はっきり見えなければ「面の角度はこの映像でははっきり確認できない」と正直に書く。なおボレーは面がやや上向き（オープン）が基本、フォアのトップスピンは擦り上げるため面がやや下向き（かぶせ）が正常、というようにショットによって適切な面の向きが異なる点を踏まえて評価する。ボレーで面が開いて見えるのを誤って「かぶせ（下向き）」と書かないこと。★最優先改善点を末尾に記載。
...
必ずJSON形式のみで返してください：
{
  "observations": {
    ...
    "contactPosition": "前で捉えている" または "やや後ろ" または "後ろ" または "確認できない"（※テイクバックの判定（takebackVsBody）からの推論で決めず、必ずインパクトの瞬間の画像を見て独立に判定する）
  },
  "formAnalysis": "...",
  "impactCheck": "...",
  "footwork": "...",
