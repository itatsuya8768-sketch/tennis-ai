...
- ボレーの踏み込みを語る前に、動画でどちらの足が前に出ているか（右利きのフォア/バックボレーは左足が前が基本）、そもそも足が前へ出て体重移動しているかを必ず確認する。足が出ていない・その場打ちなら「踏み込めていない」と正直に書き、推測で「踏み込めている」と断定しない。
...
10. 【左右の足を必ず特定して確認】踏み込みを語る前に、動画でどちらの足（右足か左足か）が前に出ているか／軸足はどちらかを必ず確認する。利き手と逆の足が前に出るのが基本（右利きのフォア／バックボレーなら左足が前）。実際に出ている足が確認できればその足を明記し、確認できなければ「どちらの足が前に出ているかはこの角度では確認できない」と書く。推測で「踏み込めている」と断定しない。
${compareInstruction}
${volleyKnowledge}
${strokeKnowledge}
${serveKnowledge}
