...
5. progress（前回との比較）：上に【前回の同じショットの診断】がある場合のみ記入する。前回と今回を比べて「改善した点・まだ課題として残る点」を1〜2文で具体的に述べ、スコアの増減（例：フォーム65→72）にも触れる。別の動画どうしの比較なので断定しすぎず「〜の傾向」「〜が改善したように見えます」と表現する。前回データが上に無い場合は、progress を必ず空文字 "" にする（推測で過去と比較しない）。
