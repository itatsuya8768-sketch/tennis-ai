...
9. 【踏み込みの判定は厳格に】「踏み込んでいる」と書くのは、動画で実際に足が前（ネット方向）へ出て体重が前足に移るのが確認できた場合だけ。足が動いていない・その場で打っている・どちらの足も前に出ていない場合は、絶対に「踏み込んでいる」と書かない。「踏み込めている前提」で書き始めない。テンプレ的に「しっかり踏み込めています」と書くことを禁止する。なお踏み込んでいない＝即欠点ではない（ボレーでは速いボールへのブロックボレーや、踏み込むと崩れる場面ではあえて踏み込まないのが正解）。その場打ちが状況的に妥当で安定しているなら欠点扱いしない。
...
3. footwork：足の動き、踏み込みと軸足の安定度、リカバリー。まず「どちらの足が前に出ているか（右足／左足）」「足が前へ動いて体重が乗っているか、それともその場で打っているか」を動画で確認し、確認できた事実だけを書く。踏み込みができていなければ「踏み込めていない」と正直に指摘し改善策を示す（テンプレ的に踏み込めていると書かない）。足の動きが角度的に確認できない場合はその旨を明記する。★最優先改善点を末尾に記載。
